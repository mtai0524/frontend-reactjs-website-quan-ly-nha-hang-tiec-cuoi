...
const History = () => {
  ...
  const repaymentInvoiceCoin = async (invoiceId) => {
    ...
    try {
      const response = await fetch(
        `http://api-wedding-palace.somee.com/api/invoice/check-repayment/${invoiceId}`,
        {
          method: "POST",
          headers: {
  ...
  const demoPayment = async (e) => {
    try {
      ...
      const response = await fetch(
        `http://api-wedding-palace.somee.com/api/Payment?amount=${amount}`
      );
      if (!response.ok) {
  ...
  const cancelInvoice = async (invoiceId) => {
    try {
      const response = await fetch(
        `http://api-wedding-palace.somee.com/api/invoice/cancel/${invoiceId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchWallet = async () => {
    try {
      const response = await fetch(
        `http://api-wedding-palace.somee.com/api/wallet/${id}`
      );
      if (!response.ok) {
  ...
  const paymentCompeleteWallet = () => {
    ...
    setTimeout(() => {
      ...
      if (storedInvoiceId) {
        ...
        fetch(
          `http://api-wedding-palace.somee.com/api/invoice/repayment-compelete-wallet/${invoiceId}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
